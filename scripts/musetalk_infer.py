...
def on_ui_tabs():
    with gr.Blocks(css=css) as musetalk_interface:
        with gr.Row():
            with gr.Column():
                audio = gr.Audio(label="Driven Audio", type="filepath")
                video = gr.Video(label="Reference Video", sources=['upload'])
                with gr.Row():
                    realtime = gr.Checkbox(label="realtime inference", elem_id="realtime inference", show_label=True,
                                           interactive=True)
                    model_id = gr.Number(label="Model ID", value=0)
                bbox_shift = gr.Number(label="BBox_shift value, px", value=0)
                bbox_shift_scale = gr.Textbox(
                    label="BBox_shift recommend value lower bound,The corresponding bbox range is generated after the initial result is generated. \n If the result is not good, it can be adjusted according to this reference value",
                    value="", interactive=False)

                btn = gr.Button("Generate")
            out1 = gr.Video()

        btn.click(fn=inference, inputs=[audio, video, bbox_shift, realtime, model_id],
                  outputs=[out1, bbox_shift_scale])
    return [(musetalk_interface, "musetalk", "musetalk")]
...
try:
    ...
    demo.queue().launch(share=False, debug=True, server_name="0.0.0.0", server_port=11245)
